...
void LcdDisplayTemp(char datas[]) 	 //lcd显示
{
	char temp[] = "Temp:";
	unsigned int i;
	for ( i = 0; i < 5; i++)
	...
	LcdWriteData(datas[9]); //十位

	LcdWriteCom(0x87);		//写地址 80表示初始地址
	LcdWriteData(datas[10]); //个位

	LcdWriteCom(0x88);		//写地址 80表示初始地址
	LcdWriteData(datas[11]); 		//显示 ‘.’

	LcdWriteCom(0x89);		 //写地址 80表示初始地址
	LcdWriteData(datas[12]); //显示小数点后的第一位  

	LcdWriteCom(0x90);		 //写地址 80表示初始地址
	LcdWriteData(datas[13]); //显示小数点后的第二位
